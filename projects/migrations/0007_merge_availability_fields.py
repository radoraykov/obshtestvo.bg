...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for member in orm.Member.objects.all():
            if member.is_active or member.is_available:
                member.availability = Member.AVAILABLE
            if member.is_paid_only:
                member.availability = Member.ONLY_PAID
            if not member.will_help:
                member.availability = Member.ONLY_READER
            member.save()

    def backwards(self, orm):
        for member in orm.Member.objects.all():
            if member.availability == Member.AVAILABLE:
                member.is_available = True
            if member.availability == Member.ONLY_PAID:
                member.is_paid_only = True
            if member.availability == Member.ONLY_READER:
                member.will_help = False
            member.save()
